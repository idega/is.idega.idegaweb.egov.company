/*
 * $Id: SubGroupEditor.java,v 1.1 2008/07/29 12:57:47 anton Exp $
...
import is.idega.idegaweb.egov.company.business.SubGroupComparator;
...
public class SubGroupEditor extends FSKBlock {
	...
	private static final String PARAMETER_PARENT_GROUP_PK = "prm_group_pk";
	private static final String PARAMETER_GROUP_PK = "prm_sub_group_pk";
			...
			switch (parseAction(iwc)) {
				...
				case ACTION_DELETE:
					if (!getBusiness().removeGroup(iwc.getParameter(PARAMETER_GROUP_PK), iwc.getCurrentUser())) {
						getParentPage().setAlertOnLoad(getResourceBundle().getLocalizedString("sub_group.remove_error", "You can not remove a group that has courses attached to it."));
					}
					showList(iwc);
					break;
			}
	...
	private void showList(IWContext iwc) throws RemoteException {
		...
		if (company != null) {
			...
			try {
				groups = new ArrayList(getBusiness().getSubGroups(company));
			}
		...
		Collections.sort(groups, new SubGroupComparator(iwc.getCurrentLocale()));
		...
		cell.add(new Text(getResourceBundle().getLocalizedString("sub_group_editor.created_date", "Created date")));
		...
		while (iter.hasNext()) {
			...
			Group divisionGroup = (Group) subGroup.getParentNode();
			Group division = divisionGroup != null ? (Group) divisionGroup.getParentNode() : null;
			...
			IWTimestamp created = new IWTimestamp(subGroup.getCreated());

			row = group.createRow();

			Link edit = new Link(getBundle().getImage("edit.png", getResourceBundle().getLocalizedString("edit", "Edit")));
			edit.addParameter(PARAMETER_GROUP_PK, subGroup.getPrimaryKey().toString());
			...
			delete.addParameter(PARAMETER_GROUP_PK, subGroup.getPrimaryKey().toString());
			delete.addParameter(PARAMETER_ACTION, ACTION_DELETE);
			delete.maintainParameter(PARAMETER_COMPANY_PK, iwc);
			delete.setClickConfirmation(getResourceBundle().getLocalizedString("sub_group.confirm_delete", "Are you sure you want to delete the group selected?"));
		...
		if (company != null && (isOpen || iwc.getIWMainApplication().getAccessController().hasRole(FSKConstants.ROLE_KEY_FSK_ADMIN, iwc))) {
			SubmitButton newLink = new SubmitButton(getResourceBundle().getLocalizedString("sub_group.new", "New sub group"), PARAMETER_ACTION, String.valueOf(ACTION_NEW));
	...
	private void showEditor(IWContext iwc, Object groupPK) throws java.rmi.RemoteException {
		...
		script2.append("function setGroups(data) {\n").append("\tDWRUtil.removeAllOptions(\"" + PARAMETER_PARENT_GROUP_PK + "\");\n").append("\tDWRUtil.addOptions(\"" + PARAMETER_PARENT_GROUP_PK + "\", data);\n").append("}");
		...
		helpLayer.add(new Text(getResourceBundle().getLocalizedString("sub_group.sub_group_editor_help", "Fill in the desired values and click 'Save'.")));
		...
		DropdownMenu groups = new DropdownMenu(PARAMETER_PARENT_GROUP_PK);
		groups.setId(PARAMETER_PARENT_GROUP_PK);
		groups.keepStatusOnAction(true);

		if (group != null) {
			Group divisionGroup = (Group) group.getParentNode();
			Group division = divisionGroup != null ? (Group) divisionGroup.getParentNode() : null;
			...
			if (divisionGroup != null) {
				groups.setSelectedElement(divisionGroup.getPrimaryKey().toString());
			}

			form.add(new HiddenInput(PARAMETER_GROUP_PK, groupPK.toString()));
		}
		if (iwc.isParameterSet(PARAMETER_DIVISION_PK)) {
			Division division = getBusiness().getDivision(iwc.getParameter(PARAMETER_DIVISION_PK));
			groups.addMenuElements(getBusiness().getGroups(division));
		}
		...
		layer = new Layer(Layer.DIV);
		layer.setStyleClass("formItem");
		label = new Label(getResourceBundle().getLocalizedString("sub_group", "Sub group"), name);
		layer.add(label);
		layer.add(name);
		section.add(layer);
	...
	private void saveGroup(IWContext iwc) throws RemoteException {
		...
		Object groupPK = iwc.isParameterSet(PARAMETER_PARENT_GROUP_PK) ? iwc.getParameter(PARAMETER_PARENT_GROUP_PK) : null;
		...
		if (groupPK == null || Integer.parseInt(groupPK.toString()) <= 0) {
			getParentPage().setAlertOnLoad(getResourceBundle().getLocalizedString("sub_group_editor_error.must_select_group", "You have to select a group"));
			showEditor(iwc, pk);
			return;
		}
		...
		if (name != null && !"".equals(name.trim())) {
			try {
				getBusiness().storeSubGroup(pk, name, groupPK, getCompany(iwc), iwc.getCurrentUser());
			}
