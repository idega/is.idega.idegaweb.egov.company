...
import is.idega.idegaweb.egov.fsk.business.BankIntegrationService;
import is.idega.idegaweb.egov.fsk.data.Constant;
import is.idega.idegaweb.egov.fsk.data.Course;
import is.idega.idegaweb.egov.fsk.data.CourseHome;
...
public class AllocationTransferPayment extends ParentAllocation {
	
	protected static final String PARAMETER_INVOICE = "prm_invoice";
	...
	protected void showForm(IWContext iwc) throws RemoteException {
		...
		if (applicant != null) {
			...
			cell = row.createHeaderCell();
			cell.setStyleClass("price");
			cell.add(new Text(iwrb.getLocalizedString("invoice_id", "Invoice number")));
			...
			while (iterator.hasNext()) {
				...
				TextInput invoice = new TextInput(PARAMETER_INVOICE);
				...
				invoice.keepStatusOnAction(true, iRow - 1);
				...
				if (isDisabled) {
					cell.add(new HiddenInput(PARAMETER_INVOICE, ""));
				}
				cell.add(hidden);
	...
	protected void showOverview(IWContext iwc) throws RemoteException {
		...
		String[] invoices = iwc.getParameterValues(PARAMETER_INVOICE);
		...
		form.maintainParameter(PARAMETER_INVOICE);
	...
	protected void save(IWContext iwc) throws RemoteException {
		...
		try {
			...
			for(int i = 0; i < allocations.length; i++) {
				course = getCourseHome().findByPrimaryKey(coursePKs[i]);
				company = course.getCompany();
				PaymentOutData out = new PaymentOutData("", "", invoices[i]);
				PaymentInData in = new PaymentInData(company.getPersonalID(), company.getBankAccount(), invoices[i], new BigDecimal(allocations[i]));
				getBankService(iwc).paySingleInvoice(out, in);
			}
		} catch(Exception e) {
			e.printStackTrace();
		}
		getBusiness(iwc).storeParentTransferAllocations(getChild(iwc), season, coursePKs, allocations);
		getSession(iwc).clear();
	...
	protected BankIntegrationService getBankService(IWApplicationContext iwac) {
		try {
			return (BankIntegrationService) IBOLookup.getServiceInstance(iwac, BankIntegrationService.class);
		}
	...
	protected CourseHome getCourseHome() {
		...
		try {
			home = (CourseHome) IDOLookup.getHome(Course.class);
		}
