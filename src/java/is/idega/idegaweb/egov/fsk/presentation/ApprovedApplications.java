/*
 * $Id: ApprovedApplications.java,v 1.1 2008/07/29 10:48:18 anton Exp $ Created on Jun 11, 2007
 * 
...
import is.idega.idegaweb.egov.fsk.business.ApplicationComparator;
...
public class ApprovedApplications extends FSKBlock {
	...
	private Table2 getApplications(IWContext iwc) throws RemoteException {
		...
		List applications = new ArrayList(getBusiness().getApprovedApplications());
		Collections.sort(applications, new ApplicationComparator(iwc, iwc.isParameterSet(PARAMETER_SORT) ? Integer.parseInt(iwc.getParameter(PARAMETER_SORT)) : ApplicationComparator.SORT_CREATED, iwc.getCurrentLocale()));
		...
		while (iterator.hasNext()) {
			...
			if (status.equals(getBusiness().getCaseStatusGranted())) {
				cell.add(new Text(getResourceBundle().getLocalizedString("application_status.open", "Open")));
			}
			else {
				cell.add(new Text(getResourceBundle().getLocalizedString("application_status.closed", "Closed")));
			}
